...
class CarState(CarStateBase, EsccCarStateBase, MadsCarState, CarStateExt):
  ...
  def update_canfd(self, can_parsers) -> tuple[structs.CarState, structs.CarStateSP]:
    ...
    if self.CP.enableBsm:
      left_bsm_sig = "FL_INDICATOR_ALT" if self.CP.carFingerprint == CAR.HYUNDAI_IONIQ_5_N else "FL_INDICATOR"
      right_bsm_sig = "FR_INDICATOR_ALT" if self.CP.carFingerprint == CAR.HYUNDAI_IONIQ_5_N else "FR_INDICATOR"
